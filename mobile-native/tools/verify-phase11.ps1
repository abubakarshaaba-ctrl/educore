...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 11 Android validation failed with exit code $LASTEXITCODE." }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 11 Laravel validation failed with exit code $LASTEXITCODE." }
} finally { Pop-Location }

Write-Host 'Phase 11 validation passed: tenant-scoped finance and operations contracts, native workspace, unit tests, Lint and debug assembly are green.' -ForegroundColor Green
