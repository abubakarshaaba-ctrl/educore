...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 5 Android validation failed with exit code $LASTEXITCODE."
    }
} finally {
    Pop-Location
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 5 Laravel validation failed with exit code $LASTEXITCODE."
    }
} finally {
    Pop-Location
}

Write-Host 'Phase 5 validation passed: Android tests, Lint, debug assembly and Laravel dashboard contracts are green.' -ForegroundColor Green
