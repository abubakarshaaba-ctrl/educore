...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 9 Android validation failed with exit code $LASTEXITCODE." }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 9 Laravel validation failed with exit code $LASTEXITCODE." }
} finally { Pop-Location }

Write-Host 'Phase 9 validation passed: repository reader/downloads, teacher-controlled lesson plans/notes, offline drafts, Android Lint and debug assembly are green.' -ForegroundColor Green
