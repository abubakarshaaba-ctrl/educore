...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 12 Android validation failed with exit code $LASTEXITCODE." }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 12 Laravel validation failed with exit code $LASTEXITCODE." }
} finally { Pop-Location }

Write-Host 'Phase 12 validation passed: tenant-scoped notices, events, messages, attachments, FCM deep links, native tests, Lint and debug assembly are green.' -ForegroundColor Green
