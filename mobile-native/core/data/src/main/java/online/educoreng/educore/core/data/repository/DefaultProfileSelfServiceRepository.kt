...
class DefaultProfileSelfServiceRepository(
    ...
    override suspend fun uploadPassport(uri: Uri): AppResult<UserProfileDto> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            if (mimeType !in ALLOWED_PASSPORT_MIME_TYPES) {
                throw IOException("Passport must be a JPG, PNG or WebP image.")
            }
            ...
            if (bytes.isEmpty()) {
                throw IOException("The selected passport image is empty.")
            }
    ...
    private companion object {
        ...
        val ALLOWED_PASSPORT_MIME_TYPES = setOf("image/jpeg", "image/jpg", "image/png", "image/webp")
    }
}
