...
/**
 * Canonical single-select dropdown for finance, academics and administration.
 * This replaces feature-local dropdown implementations so selection controls
 * share the same touch target, typography and action hierarchy everywhere.
 */
@Composable
fun <T> EduCoreDropdownField(
    label: String,
    options: List<T>,
    selected: T?,
                ...
                options.forEach { option ->
                    DropdownMenuItem(
                        ...
                        trailingIcon = if (option == selected) {
                            { Text("Selected", style = MaterialTheme.typography.labelSmall, color = EduCoreColors.Gold700) }
                        } else {
                            null
                        },
                        onClick = {
