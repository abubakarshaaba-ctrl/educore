...
class MessagingController extends Controller
{
    ...
    public function inbox(Request $request)
    {
        ...
        $threads = MessageThread::query()
            ...
            ->when(! $this->canOverseeAllThreads($user), function ($query) use ($user, $userId): void {
                $audiences = $this->broadcastAudiencesFor($user);
                $query->where(function ($visible) use ($audiences, $userId): void {
            ...
            ->with(['student', 'initiator', 'recipient', 'replies' => fn ($query) => $query->latest()->limit(1)])
            ->latest('updated_at')
            ->paginate(20);

        $unreadCount = collect($threads->items())->filter(fn (MessageThread $thread) => $this->threadIsUnreadFor($thread, $userId))->count();
    ...
    public function storeInternal(Request $request)
    {
        ...
        $data = $request->validate([
            'target_type' => ['required', Rule::in($allowedTargets)],
        ...
        if (in_array($data['target_type'], ['staff', 'parent'], true)) {
            ...
            if ($data['target_type'] === 'staff') {
                abort_unless($recipient->isTenantStaff(), 422, 'Select a valid staff member.');
            } else {
                abort_unless($recipient->isParent(), 422, 'Select a valid parent.');
            }
        ...
        $thread = DB::transaction(function () use ($data, $recipient, $audience, $user): MessageThread {
            $thread = MessageThread::create([
                'tenant_id' => $this->tenantId(),
                'student_id' => null,
                'conversation_type' => $data['target_type'],
        ...
        return redirect()->route('messages.thread', $thread)
            ->with('success', $audience ? 'Broadcast sent.' : 'Message sent.');
    }
        ...
        if ($thread->isBroadcast() && (int) $thread->initiated_by !== (int) $user->id) {
            $private = DB::transaction(function () use ($thread, $data, $user): MessageThread {
                $private = MessageThread::create([
                    'tenant_id' => $this->tenantId(),
                    'student_id' => null,
                    'conversation_type' => $user->isParent() ? 'parent' : 'staff',
                    'recipient_user_id' => $thread->initiated_by,
                    'audience' => null,
            ...
            return redirect()->route('messages.thread', $private)
                ->with('success', 'Your reply was sent privately to school administration.');
        }
