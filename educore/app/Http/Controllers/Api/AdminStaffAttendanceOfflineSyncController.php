    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        try {
            $record = DB::transaction(function () use ($actor, $staff, $settings, $data, $timestamp) {
                // Serialize mutations of an existing daily row where supported.
                ...
                // Kept for legacy reporting only; the sync-event ledger is the
            ...
            // A simultaneous retry can win the unique (tenant_id, client_uuid)
            // insert. The losing transaction rolls back, then returns the
            // authoritative event instead of surfacing a 500.
            if ($existing = $this->existingEvent((int) $actor->tenant_id, $data['client_uuid'])) {
                return $this->idempotentResponse($existing);
            }
            throw $exception;
