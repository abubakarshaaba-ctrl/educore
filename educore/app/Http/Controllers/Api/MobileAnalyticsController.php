...
/**
 * Read-only analytics contract for the native app.
 *
 * This intentionally returns the same metrics/sections envelope consumed by
 * OperationsScreen, while keeping analytics authorization and aggregation
 * separate from operational registers.
 */
class MobileAnalyticsController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $averageScore = Schema::hasTable('termly_summaries')
            ? TermlySummary::query()
                ->where('tenant_id', $tenantId)
                ->when($termIds->isNotEmpty(), fn ($query) => $query->whereIn('term_id', $termIds))
                ->avg('final_average')
            : null;
        ...
        $attendanceRate = ($attendance?->total ?? 0) > 0
            ? round(((int) $attendance->present / (int) $attendance->total) * 100, 1)
            : null;
    ...
    {
        if (! Schema::hasTable('termly_summaries') || ! Schema::hasTable('class_arms') || ! Schema::hasTable('class_levels')) {
            return [];
        }

        return TermlySummary::query()
            ->where('termly_summaries.tenant_id', $tenantId)
            ->when($termIds !== [], fn ($query) => $query->whereIn('termly_summaries.term_id', $termIds))
            ->join('class_arms', function ($join) use ($tenantId) {
                $join->on('class_arms.id', '=', 'termly_summaries.class_arm_id')
                    ->where('class_arms.tenant_id', '=', $tenantId);
            })
            ->join('class_levels', function ($join) use ($tenantId) {
                $join->on('class_levels.id', '=', 'class_arms.class_level_id')
                    ->where('class_levels.tenant_id', '=', $tenantId);
            })
            ...
            ->map(fn ($row) => $this->record(
                'class-'.$row->class_id,
                trim($row->level_name.' '.$row->arm_name),
                number_format((float) $row->average_score, 1).'% average',
                null,
                [
                    'Students' => (string) $row->students,
                    'Passed all subjects' => (string) $row->passed_all,
                    'Average' => number_format((float) $row->average_score, 1).'%',
                ],
            ))
            ->values()
            ->all();
    ...
    {
        if (! Schema::hasTable('scores') || ! Schema::hasTable('subjects')) {
            return [];
        }

        return Score::query()
            ->where('scores.tenant_id', $tenantId)
            ->when($termIds !== [], fn ($query) => $query->whereIn('scores.term_id', $termIds))
            ->join('subjects', function ($join) use ($tenantId) {
            ...
            ->map(fn ($row) => $this->record(
                ...
                [
                    'Average' => number_format((float) $row->average_score, 1).'%',
                    'Lowest' => number_format((float) $row->minimum_score, 1).'%',
                    'Highest' => number_format((float) $row->maximum_score, 1).'%',
                    'Score records' => (string) $row->attempts,
                ],
            ))
            ->values()
            ->all();
    ...
    private function enrollmentTrend(int $tenantId): array
    {
        ...
        $monthly = Student::query()
            ->where('tenant_id', $tenantId)
            ->whereBetween('created_at', [$start, $end])
            ->get(['id', 'created_at'])
            ->groupBy(fn (Student $student) => $student->created_at?->format('Y-m') ?? 'unknown')
            ->reject(fn ($items, $month) => $month === 'unknown')
            ->map(fn ($items, $month) => [
                'month' => $month,
                'count' => $items->count(),
            ])
        ...
        return $monthly->map(fn ($row) => $this->record(
            'enrollment-'.$row['month'],
            \Carbon\Carbon::createFromFormat('Y-m', $row['month'])->format('F Y'),
            $row['count'].' new student'.($row['count'] === 1 ? '' : 's'),
            null,
            ['Enrolled' => (string) $row['count']],
        ))->values()->all();
    }
    ...
    private function finance(int $tenantId, ?int $sessionId): array
    {
        $query = Invoice::query()
            ->where('tenant_id', $tenantId)
            ->when($sessionId, fn ($builder) => $builder->where('session_id', $sessionId));
        ...
        return [
            ...
            $this->section('finance', 'Fee overview', [
                $this->record('finance-current', 'Current session fees', $rate !== null ? number_format($rate, 1).'% collected' : 'No billing data', null, [
    ...
    private function section(string $key, string $title, array $records): array
    {
        return compact('key', 'title') + ['count' => count($records), 'records' => $records];
    }
    ...
    private function record(string $id, string $title, ?string $subtitle, ?string $status, array $fields): array
    {
        return [
            ...
            'subtitle' => $subtitle,
            'status' => $status,
            'fields' => collect($fields)->map(fn ($value, $label) => [
                'label' => (string) $label,
                'value' => (string) $value,
            ])->values()->all(),
        ];
