...
class MobileReportsController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'selected' => ['class_arm_id' => $classArm?->id, 'term_id' => $term?->id],
            'class' => $classArm ? ['id' => $classArm->id, 'name' => trim(($classArm->classLevel?->name ?? '').' '.$classArm->name)] : null,
    ...
    public function compute(Request $request): JsonResponse
    {
        ...
        $computed = $this->computation->compute($tenantId, (int) $data['class_arm_id'], (int) $data['term_id']);
        return response()->json(['message' => "{$computed} report card(s) computed.", 'computed' => $computed]);
    }
    ...
    public function publish(Request $request): JsonResponse
    {
        ...
        $result = $this->publication->publish($tenantId, (int) $data['class_arm_id'], (int) $data['term_id'], $user, isset($data['note']) ? (string) $data['note'] : null, $request);
        return response()->json([
    ...
    public function unpublish(Request $request): JsonResponse
    {
        ...
        $tenantId = (int) $user->tenant_id;
        $data = $this->validateSelection($request, $tenantId);
        $publication = $this->publication->unpublish($tenantId, (int) $data['class_arm_id'], (int) $data['term_id'], $user, $request);
        return response()->json([
    ...
    private function validateSelection(Request $request, int $tenantId, bool $includeNote = false): array
    {
        ...
        if ($includeNote) $rules['note'] = ['nullable', 'string', 'max:2000'];
    ...
    private function summaryPayload(TermlySummary $summary): array
    {
        ...
        return [
            ...
            'student' => [
                ...
                'name' => $student ? trim(implode(' ', array_filter([$student->first_name, $student->middle_name, $student->last_name]))) : 'Student #'.$summary->student_id,
    ...
    {
        return in_array($user->roleKey(), self::ADMIN_ROLES, true) || $user->canAccessModule('reports.remarks');
    }
}
