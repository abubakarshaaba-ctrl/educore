...
class AdminController extends Controller
{
    ...
    public function updateStudent(Request $request, Student $student)
    {
        ...
        return response()->json(['message' => 'Student updated.', 'status' => $student->status]);
    }
    ...
    public function updateStaff(Request $request, User $member)
    {
        ...
        $data = $request->validate([
            ...
            'role' => ['sometimes', Rule::in(User::staffRoleNames())],
        ]);

        if (isset($data['role'])) {
            $data['role'] = User::canonicalRole($data['role']);
        }
    ...
    {
        $user = $this->administrator($request);
        abort_unless($this->allowsAny($user, ['students', 'staff', 'classes', 'subjects']), 403);
    ...
    {
        $user = $this->administrator($request);
        abort_unless($this->allows($user, 'classes'), 403);

        $arm = ClassArm::create([...$this->classData($request, $user), 'tenant_id' => $user->tenant_id]);
    ...
    {
        $user = $this->administrator($request);
        abort_unless((int) $classArm->tenant_id === (int) $user->tenant_id && $this->allows($user, 'classes'), 404);
    ...
    {
        $user = $this->administrator($request);
        abort_unless($this->allows($user, 'subjects'), 403);
    ...
    {
        $user = $this->administrator($request);
        abort_unless((int) $subject->tenant_id === (int) $user->tenant_id && $this->allows($user, 'subjects'), 404);
