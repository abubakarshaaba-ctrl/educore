...
/**
 * Native export gateway.
 *
 * GET /api/v1/operations/exports returns available export choices.
 * Add action=download&type=... to stream an authorized CSV using the same
 * business meaning as the web exports, with explicit tenant boundaries.
 */
class MobileExportsController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        return response()->json($this->options($user));
    }
    ...
    private function options(User $user): array
    {
        ...
        $classes = ($canStudents || $canBroadsheet)
            ? ClassArm::query()
        ...
        return [
            ...
            'module' => [
                'key' => 'exports',
                'title' => 'Exports',
                'description' => 'Generate tenant-scoped CSV reports for authorized school data',
                'mobile_policy' => 'native_download',
            ],
            'capabilities' => [
                'students' => $canStudents,
                'broadsheet' => $canBroadsheet,
                'fees' => $canFees,
            ],
            'classes' => $classes,
            'terms' => $terms,
    ...
    private function download(Request $request, User $user): StreamedResponse
    {
        $data = $request->validate([
            'type' => ['required', Rule::in(['students', 'broadsheet', 'fees'])],
        ]);
    ...
    private function studentsCsv(Request $request, User $user): StreamedResponse
    {
        ...
        return $this->csv($filename, ['Admission No', 'Last Name', 'First Name', 'Gender', 'DOB', 'Class', 'Status'], function ($file) use ($students): void {
    ...
    private function feesCsv(Request $request, User $user): StreamedResponse
    {
        ...
        $validated = $request->validate([
            'session_id' => [
                'nullable',
                'integer',
                Rule::exists('academic_sessions', 'id')->where(fn ($query) => $query->where('tenant_id', $tenantId)),
            ],
        ]);
        ...
        $invoices = Invoice::query()
            ->where('tenant_id', $tenantId)
            ->with('student')
            ->when($validated['session_id'] ?? null, fn ($query, $sessionId) => $query->where('session_id', $sessionId))
            ->orderBy('invoice_number')
            ->get();

        $filename = 'Fees_Report_'.now()->format('Y-m-d').'.csv';

        return $this->csv($filename, ['Invoice No', 'Student', 'Amount Billed', 'Amount Paid', 'Balance', 'Status'], function ($file) use ($invoices): void {
            foreach ($invoices as $invoice) {
                fputcsv($file, [
                    ...
                    $invoice->amount_paid,
                    max(0, (float) $invoice->total_amount - (float) $invoice->amount_paid),
                    $invoice->status,
                ]);
            }
        });
    ...
    private function canBroadsheet(User $user): bool
    {
        return $user->canAccessModule('scores')
            || $user->canAccessModule('scores.view')
            || $user->canAccessModule('reports');
    }
