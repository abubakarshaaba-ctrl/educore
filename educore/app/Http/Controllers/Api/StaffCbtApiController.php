...
/**
 * Native staff CBT management contract.
 *
 * This endpoint is intentionally separate from MobileCbtController, whose
 * contract is student-attempt oriented. Staff access follows the same rules
 * as the web CBT manager: administrators/full CBT roles can see all tenant
 * exams, while teacher roles are scoped to subjects/classes they teach.
 */
class StaffCbtApiController extends Controller
{
    ...
    public function show(Request $request, CbtExam $exam)
    {
        ...
        $submitted = CbtStudentSession::where('cbt_exam_id', $exam->id)
            ->whereNotNull('submitted_at')
            ->count();
        $graded = CbtStudentSession::where('cbt_exam_id', $exam->id)
    ...
    public function publish(Request $request, CbtExam $exam, CbtExamConfigurationService $configuration)
    {
        ...
        if ($errors) {
            return response()->json([
                'message' => 'The exam is not ready to publish.',
                'errors' => $errors,
            ], 422);
        }
    ...
    private function payload(CbtExam $exam): array
    {
        ...
        return [
            ...
            'scheduled_start' => $exam->scheduled_start?->toIso8601String(),
            'scheduled_end' => $exam->scheduled_end?->toIso8601String(),
            'subject' => $exam->questionBank?->subject ? [
                'id' => $exam->questionBank->subject->id,
                'name' => $exam->questionBank->subject->name,
            ] : null,
            'class_level' => $exam->questionBank?->classLevel ? [
                'id' => $exam->questionBank->classLevel->id,
                'name' => $exam->questionBank->classLevel->name,
            ] : null,
            'classes' => $exam->classArms->map(fn ($arm) => [
                'id' => $arm->id,
            ...
            'term' => $exam->term ? [
                'id' => $exam->term->id,
                'name' => $exam->term->name,
                'session' => $exam->term->session?->name,
            ] : null,
