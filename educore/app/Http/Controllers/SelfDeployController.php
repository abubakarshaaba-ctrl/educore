...
class SelfDeployController extends Controller
{
    ...
    /**
     * Temporary diagnostic: attempts a real exam-period creation on this
     * server and reports the exact exception instead of a blank 500.
     * Remove once the exam-period 500 is confirmed fixed.
     */
    public function diagExam(Request $request)
    {
        $expected = (string) (config('app.deploy_token') ?: self::derivedToken());
        if ($expected === '' || !hash_equals($expected, (string) $request->query('token'))) {
            abort(403);
        ...
        $out = [];
        try {
            $user = \App\Models\User::where('is_super_admin', 0)->whereNotNull('tenant_id')->first();
            if (!$user) {
                return response()->json(['ok' => false, 'step' => 'find-user', 'message' => 'No non-super-admin staff user found.']);
            }
            \Illuminate\Support\Facades\Auth::login($user);
            $out['tenant_id'] = $user->tenant_id;

            $term = \App\Models\Term::where('tenant_id', $user->tenant_id)->first();
            $level = \App\Models\ClassLevel::where('tenant_id', $user->tenant_id)->first();
            $out['term_id'] = $term?->id;
            $out['class_level_id'] = $level?->id;

            if (!$term || !$level) {
                return response()->json(['ok' => false, 'step' => 'find-data', 'out' => $out, 'message' => 'No term or class level found for this tenant.']);
            }

            $req = new Request();
            $req->replace([
                'term_id' => $term->id,
                'title' => 'Diagnostic Exam ' . now()->timestamp,
                'start_date' => now()->addDays(2)->toDateString(),
                'end_date' => now()->addDays(10)->toDateString(),
                'excluded_weekdays' => [0, 6],
                'sessions' => [['name' => 'Morning', 'start_time' => '08:00', 'end_time' => '10:00']],
                'class_level_ids' => [$level->id],
            ]);

            $ctrl = app(\App\Http\Controllers\ExamPeriodController::class);
            $res = $ctrl->store($req);
            $out['result_class'] = get_class($res);

            $period = \App\Models\ExamPeriod::where('title', 'like', 'Diagnostic Exam %')->latest()->first();
            ...
            return response()->json([
                ...
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => collect($e->getTrace())->take(6)->map(fn ($t) => ($t['file'] ?? '?') . ':' . ($t['line'] ?? '?') . ' ' . ($t['function'] ?? ''))->all(),
            ]);
        }
    }
}
