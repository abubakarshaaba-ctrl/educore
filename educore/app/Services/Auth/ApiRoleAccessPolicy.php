...
class ApiRoleAccessPolicy
{
    ...
    public function allows(User $user, Request $request): bool
    {
        ...
        // Portal users are isolated by their own controllers and tenant-linked
        // student/guardian resolution. Do not apply staff role contracts to them.
        ...
        if ($path === 'auth/logout' || $path === 'bootstrap' || $path === 'me') {
            return true;
        }
        ...
        if ($path === 'portal/modules' || $path === 'portal/session') {
            return true;
        }

        if (preg_match('#^(push|devices)/(register|unregister)$#', $path)) {
            return true;
        }
        ...
        if (str_starts_with($path, 'exam-duties')) {
            return $this->allowsAny($user, ['timetable', 'timetable.view', 'dashboard']);
        }
        ...
        if (str_starts_with($path, 'notifications')) {
            return $this->allowsAny($user, ['notifications', 'notifications.send', 'notifications.view']);
        }
        ...
        if (str_starts_with($path, 'admin/staff-attendance')) {
            return $this->allowsAny($user, ['staff-attendance']);
        }

        if (str_starts_with($path, 'admin/students')) {
            return $this->allowsAny($user, ['students']);
        }

        if (str_starts_with($path, 'admin/staff')) {
            return $this->allowsAny($user, ['staff']);
        }

        if (str_starts_with($path, 'admin/academics')) {
            return $this->allowsAny($user, ['classes', 'subjects', 'curriculum', 'academic-cycle']);
        }

        if (str_starts_with($path, 'admin/finance')) {
            return $this->allowsAny($user, ['fees', 'expenses', 'payroll']);
        }

        if (str_starts_with($path, 'admin/management')) {
            return $this->allowsAny($user, ['settings', 'subjects', 'classes']);
        }

        if (str_starts_with($path, 'accountant')) {
            return $this->allowsAny($user, ['fees', 'expenses', 'payroll']);
        }
        ...
        // Unknown tenant-staff API surfaces are never implicitly authorized.
        return false;
    }
    ...
    private function allowsAny(User $user, array $modules): bool
    {
        foreach ($modules as $module) {
            if ($this->allowsModule($user, $module)) {
                return true;
            }
        }
    ...
    private function allowsModule(User $user, string $module): bool
    {
        ...
        if (
            ...
            && in_array($module, self::ADMISSION_OFFICER_DEFAULT_DENIES, true)
        ) {
            return false;
        }

        // Exact permission first. Parent module access is accepted only when the
        // role was explicitly granted a scoped child permission (e.g. scores.entry).
        if ($user->canAccessExactModule($module)) {
            return true;
        }

        $effective = $user->effectivePermissionKeys();
        ...
        // Academic repository is intentionally available to administrators and
        // teaching roles even though it predates the ROLE_ACCESS key matrix.
