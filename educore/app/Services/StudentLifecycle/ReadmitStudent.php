...
class ReadmitStudent
{
    ...
    public function execute(User $actor, Student $student, array $data, ?Request $request = null): Student
    {
        ...
        if (!$actor->can('student.readmit')) {
            throw ValidationException::withMessages([
                'student' => 'You do not have permission to readmit students.',
            ]);
        }
        ...
        return DB::transaction(function () use ($actor, $student, $data, $request, $tenantId) {
            ...
            if ($lockedStudent->status !== Student::STATUS_TRANSFERRED_OUT) {
                throw ValidationException::withMessages([
                    'student' => 'Only transferred-out students can be readmitted.',
                ]);
            }
            ...
            if ($currentEnrollments->isNotEmpty()) {
                throw ValidationException::withMessages([
                    'student' => 'This student already has a current enrolment. Run enrolment repair before readmission.',
                ]);
            }
            ...
            $history = StudentStatusHistory::create([
                'tenant_id' => $tenantId,
                'student_id' => $lockedStudent->id,
                'old_status' => Student::STATUS_TRANSFERRED_OUT,
