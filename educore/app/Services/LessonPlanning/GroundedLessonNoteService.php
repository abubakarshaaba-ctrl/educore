...
class GroundedLessonNoteService
{
    public function __construct(
        private LessonAiProvider $provider,
        private CurriculumRetrievalService $retrieval,
        private WebLessonResearchService $webResearch,
        private LessonPromptFactory $prompts,
        private LessonNoteSchema $schema,
        private LessonNoteValidationService $validator,
        private StructuredNoteRenderer $renderer,
        private CanonicalRepositoryStudentNoteService $canonicalRepository,
    ) {}

    public function generate(LessonPlan $plan, int $userId, string $depth = 'standard', ?array $onlyItems = null): LessonNoteRevision
    {
        if (! $plan->isPublished()) {
            throw new \DomainException('Publish the lesson plan before generating its student note.');
        }

        $plan->loadMissing(['subject', 'classLevel', 'term']);

        // Primary path: build directly from the platform Academic Repository.
        // This is deterministic, uses only indexed canonical source material and
        // does not require an AI provider or external web research.
        $canonical = $this->canonicalRepository->build($plan, $depth);
        if ($canonical !== null) {
            if ($onlyItems && ($previous = $plan->noteRevisions()->latest('revision')->first())) {
                $canonical = $this->mergeMissing($previous->content, $canonical);
            }

            $context = $canonical['source_trace'] ?? [];
            $validation = $this->validator->validate($plan, $canonical, $context);
        ...
        // Compatibility fallback: if no canonical repository evidence exists for
        // this lesson yet, retain the existing grounded AI workflow. EduCore is
        // therefore not dependent on AI when repository knowledge is available.
        ...
        try {
            $usage = $this->provider->generateStructured(
                $this->prompts->noteSystem(),
                $this->prompts->noteUser($plan, $context, $depth, $onlyItems),
                $this->schema->jsonSchema(),
                $depth === 'detailed' ? 6000 : ($depth === 'concise' ? 2600 : 4200),
            );
            ...
            if ($onlyItems && ($previous = $plan->noteRevisions()->latest('revision')->first())) {
                $content = $this->mergeMissing($previous->content, $content);
            }
            ...
            // Usage telemetry applies only to the AI fallback path and must never
            // turn an otherwise successful note generation into HTTP 500.
            try {
                AiUsageLog::create([
                    ...
                    'request_type' => $onlyItems ? 'regenerate_missing_sections' : 'generate_lesson_note',
    ...
    private function persistRevision(
        ...
        return DB::transaction(function () use ($plan, $userId, $depth, $content, $validation, $aiGenerated) {
            ...
            LessonNoteValidation::create(array_merge($validation, [
                'tenant_id' => $plan->tenant_id,
                'lesson_plan_id' => $plan->id,
                'lesson_note_revision_id' => $revision->id,
            ]));
            $plan->update([
    ...
    {
        $sections = collect($existing['sections'] ?? [])->keyBy(fn ($section) => mb_strtolower($section['heading'] ?? ''));
        foreach ($generated['sections'] ?? [] as $section) {
            $sections->put(mb_strtolower($section['heading'] ?? uniqid()), $section);
        }
        $existing['sections'] = $sections->values()->all();

        foreach (['evaluation', 'source_trace'] as $key) {
            $existing[$key] = collect($existing[$key] ?? [])
                ->merge($generated[$key] ?? [])
                ->unique(fn ($value) => json_encode($value))
                ->values()
                ->all();
        }
        ...
        if (! empty($generated['generation'])) {
            $existing['generation'] = $generated['generation'];
        }
