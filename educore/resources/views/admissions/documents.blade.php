    ...
    <div style="padding:14px 20px;border-bottom:1px solid var(--border);background:#F8FAFC;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        <div>
            <div style="font-size:14px;font-weight:700;color:var(--midnight)">
                Documents — {{ $admission->first_name }} {{ $admission->last_name }}
            </div>
            <div style="font-size:12px;color:var(--slate-light);margin-top:2px">{{ $admission->application_number }}</div>
        </div>
    ...
    <div style="padding:20px">
        ...
        <div class="doc-card">
            ...
            <div class="doc-actions">
                ...
                <a href="{{ route('admissions.documents.download', [$admission, $doc]) }}" class="btn btn-download">
                    ⬇ Download
                </a>

                @if(!$doc->isVerified())
                <button type="button" class="btn btn-verify"
                    onclick="openVerifyModal({{ $doc->id }}, 'verified', '{{ route('admissions.documents.verify', [$admission, $doc]) }}')">
                    ✓ Verify
                </button>
                @endif
                ...
                <button type="button" class="btn btn-reject"
                    onclick="openVerifyModal({{ $doc->id }}, 'rejected', '{{ route('admissions.documents.verify', [$admission, $doc]) }}')">
                    ✗ Reject
                </button>
                @endif

                @if(!$doc->isPending())
                <form method="POST" action="{{ route('admissions.documents.verify', [$admission, $doc]) }}" style="display:inline">
                    @csrf
        ...
        <div style="text-align:center;padding:60px 20px;color:var(--slate-light)">
            ...
            <div style="font-size:12px;margin-top:4px">This application has no attached files.</div>
        </div>
        @endforelse
    </div>
</div>
