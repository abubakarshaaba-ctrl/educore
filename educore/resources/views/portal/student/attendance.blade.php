...
@if($parallelAttendance->isNotEmpty())
<h2 style="font-size:17px;font-weight:800;margin:24px 0 14px">📘 Parallel Attendance</h2>
@foreach($parallelAttendance as $programme)
@php($ps=$programme['stats'])
<div class="card" style="margin-bottom:16px">
    ...
    <div class="kpi-row" style="padding:14px 14px 0">
        <div class="kpi"><div class="kv">{{ $ps['total'] }}</div><div class="kl">Days</div></div>
        <div class="kpi"><div class="kv" style="color:#059669">{{ $ps['present'] }}</div><div class="kl">Present</div></div>
        <div class="kpi"><div class="kv" style="color:#DC2626">{{ $ps['absent'] }}</div><div class="kl">Absent</div></div>
        <div class="kpi"><div class="kv" style="color:#D97706">{{ $ps['late'] }}</div><div class="kl">Late</div></div>
        <div class="kpi"><div class="kv">{{ $ps['excused'] }}</div><div class="kl">Excused</div></div>
        <div class="kpi"><div class="kv">{{ $ps['rate'] }}%</div><div class="kl">Rate</div></div>
    </div>
    <div class="attendance-table">
        <table>
            <thead><tr><th>Date</th><th>Day</th><th>Status</th><th>Remark</th></tr></thead>
            <tbody>
            @forelse($programme['records'] as $r)
            <tr>
                <td style="font-weight:600">{{ $r->attendance_date?->format('d M Y') }}</td>
                <td style="color:var(--muted)">{{ $r->attendance_date?->format('l') }}</td>
                <td><span class="badge {{ $r->status==='present'?'b-g':($r->status==='absent'?'b-r':'b-a') }}">{{ ucfirst($r->status) }}</span></td>
                <td style="font-size:12px;color:var(--muted)">{{ $r->remark ?? '—' }}</td>
            </tr>
            @empty
            <tr><td colspan="4" class="empty">No parallel attendance records found.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
