...
.topnav { position:sticky; top:0; z-index:50; background:rgba(11,29,58,0.97); backdrop-filter:blur(12px); border-bottom:1px solid rgba(255,255,255,0.07); padding:0 24px; display:flex; align-items:center; justify-content:space-between; gap:12px; height:60px; min-width:0; }
...
.topnav-name { font-size:14px; font-weight:700; color:white; letter-spacing:-0.01em; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
...
.hero h1 { font-size:clamp(26px, 5vw, 44px); font-weight:800; letter-spacing:-0.03em; margin-bottom:14px; overflow-wrap:anywhere; }
...
.section { max-width:820px; margin:0 auto; padding:48px 24px; width:100%; }
.postings { display:flex; flex-direction:column; gap:16px; min-width:0; }
.posting-card { background:white; border:1px solid var(--border); border-radius:14px; padding:24px; display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap; transition:box-shadow 150ms, transform 150ms; min-width:0; }
.posting-card:hover { box-shadow:0 8px 24px rgba(0,0,0,0.06); transform:translateY(-2px); }
.posting-card>div{min-width:0;flex:1 1 300px}
.posting-title { font-size:17px; font-weight:700; color:var(--navy); margin-bottom:4px; overflow-wrap:anywhere; }
.posting-meta { font-size:13px; color:var(--slate); overflow-wrap:anywhere; }
.btn-view { display:inline-flex; align-items:center; justify-content:center; gap:6px; padding:10px 20px; font-size:13px; font-weight:700; border-radius:10px; text-decoration:none; background:var(--blue); color:white; flex-shrink:0; }
...
.portal-footer { background:var(--navy); color:rgba(255,255,255,0.45); text-align:center; padding:26px 24px; font-size:12px; overflow-wrap:anywhere; }
@media(max-width:640px){
    .topnav{padding:0 14px;height:56px}
    .topnav-logo{width:32px;height:32px}
    .topnav-name{max-width:150px;font-size:13px}
    .nav-link{padding:6px 8px;font-size:12px}
    .hero{padding:46px 16px 40px}
    .hero-sub{font-size:14px}
    .section{padding:28px 14px}
    .posting-card{padding:18px;align-items:stretch}
    .posting-card>div{flex-basis:100%}
    .btn-view{width:100%;min-height:42px}
    .empty{padding:42px 16px}
}
@media(max-width:380px){
    .topnav-name{max-width:110px}
