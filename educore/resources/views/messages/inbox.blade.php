...
$hasUnread=(bool)($thread->is_unread_for_user ?? false);
$label=match($thread->conversation_type ?? 'student'){'all_staff'=>'All Staff','all_parents'=>'All Parents','staff'=>'Staff conversation','parent'=>'Parent conversation','admin'=>'School Administration',default=>optional($thread->student)->full_name ?? optional($thread->recipient)->name ?? 'School conversation'};
@endphp
<a href="{{ route('messages.thread',$thread) }}" class="thread-item {{ $hasUnread?'unread':'' }}"><div class="th-av">{{ strtoupper(substr($label,0,1)) }}</div><div style="flex:1;min-width:0"><div class="th-subject">{{ $thread->subject }}</div><div class="th-preview">{{ $lastReply ? \App\Support\EduCoreRichText::plainText($lastReply->body) : 'No messages yet' }}</div><div class="th-meta">{{ $label }} · {{ ucfirst($thread->status) }} @if($thread->audience)· Broadcast @endif</div></div>@if($hasUnread)<div class="unread-dot"></div>@endif<div class="th-time">{{ $thread->updated_at->diffForHumans(null,true) }}</div></a>
@empty<div class="empty"><div style="font-size:32px;margin-bottom:10px">💬</div><div style="font-size:14px;font-weight:600;color:var(--midnight)">No messages yet</div><div style="font-size:13px;margin-top:4px">Start a school message or a student-linked conversation.</div></div>@endforelse
</div>{{ $threads->links() }}
@endsection
