...
@push('styles')
<style>
.grade-layout{display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:18px;align-items:start}
.grade-intro{margin-bottom:16px;padding:16px 18px;border:1px solid #E2E8F0;border-radius:13px;background:#fff;display:flex;justify-content:space-between;gap:18px;align-items:center}.grade-intro h2{margin:0 0 4px;color:#071E45;font-size:17px}.grade-intro p{margin:0;color:#64748B;font-size:11px;line-height:1.5}.grade-count{flex:0 0 auto;padding:7px 10px;border-radius:20px;background:#FFF8E8;color:#8A5B00;font-size:10px;font-weight:800}
...
@media(max-width:1024px){.grade-layout{grid-template-columns:1fr}.form-card{position:static}.level-grid{grid-template-columns:1fr}}
@media(max-width:640px){.grade-intro{align-items:flex-start}.grade-count{display:none}.level-grid{padding:10px}.form-row{grid-template-columns:1fr}.section-group summary{padding:12px}.grade-table th,.grade-table td{padding:7px 8px}}
</style>
@endpush
...
<div class="grade-intro">
    <div>
        ...
        <p>Classes are grouped by school section so related levels can use and review their grading thresholds together.</p>
    </div>
...
<div class="grade-layout">
                    ...
                    @foreach($sectionLevels as $level)
                        ...
                        <div class="level-card">
                            <div class="level-head">
                                ...
                                <span class="level-count">{{ $levelGrades->count() }} grades</span>
                            </div>
                            ...
                            @else
                                <div class="grade-table-wrap">
                                    <table class="grade-table">
                                        <thead><tr><th>Grade</th><th>Range</th><th>Remark</th><th>Pass</th><th>GP</th><th></th></tr></thead>
                                        <tbody>
                                            @foreach($levelGrades as $g)
                                                <tr>
                                                    <td class="grade-letter">{{ $g->grade_letter }}</td>
                                                    <td class="range">{{ $g->min_score }}–{{ $g->max_score }}</td>
                                                    <td>{{ $g->remark }}</td>
                                                    <td><span class="{{ $g->is_pass_grade ? 'pass' : 'fail' }}">{{ $g->is_pass_grade ? 'Yes' : 'No' }}</span></td>
                                                    <td>{{ $g->grade_point }}</td>
                                                    <td>
                                                        <form method="POST" action="{{ route('classes.grading.destroy',$g) }}" onsubmit="return confirm('Remove this grade entry?')">
                                                            @csrf @method('DELETE')
                                                            <button type="submit" class="delete-btn" aria-label="Remove {{ $g->grade_letter }} from {{ $level->name }}">Remove</button>
                                                        </form>
                                                    </td>
                                                </tr>
                                            @endforeach
    ...
    <aside class="form-card">
        <div class="form-head"><h3>Add or update grade</h3><p>Select a class level and define one score band at a time. Reusing the same grade letter updates that entry.</p></div>
        <div class="form-body">
            <form method="POST" action="{{ route('classes.grading.store') }}">
                @csrf
                <div class="fg">
                    <label class="fl">Class level <span class="req">*</span></label>
                    <select name="class_level_id" class="fc" required>
                        <option value="">Select class level</option>
                        @foreach($groupedLevels as $section => $sectionLevels)
                            <optgroup label="{{ $sectionLabels[$section] ?? ucwords(str_replace('_',' ',$section)) }}">
                                @foreach($sectionLevels as $level)
                                    <option value="{{ $level->id }}" {{ (string)old('class_level_id') === (string)$level->id ? 'selected' : '' }}>{{ $level->name }}</option>
                                @endforeach
                            </optgroup>
                        @endforeach
                    </select>
