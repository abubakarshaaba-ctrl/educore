...
<div {{ $attributes->merge(['class' => 'auth-footer', 'style' => 'display:flex;flex-direction:column;align-items:center;gap:14px']) }}>
    <nav class="auth-footer__contacts" aria-label="EduCore support contacts" style="display:flex;flex-direction:column;align-items:flex-start;gap:8px">
        <a href="tel:{{ $supportPhoneInternational }}" style="display:flex;align-items:center;gap:8px;color:var(--ec-muted);font-weight:650;text-decoration:none">
        ...
        <a href="https://wa.me/2347065595768" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:8px;color:var(--ec-muted);font-weight:650;text-decoration:none">
        ...
        <a href="mailto:{{ $supportEmail }}" style="display:flex;align-items:center;gap:8px;color:var(--ec-muted);font-weight:650;text-decoration:none">
            <svg viewBox="0 0 24 24" fill="currentColor" style="width:15px;height:15px;flex-shrink:0"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>
            {{ $supportEmail }}
        </a>
    </nav>
    <div class="auth-footer__meta" style="display:flex;justify-content:center;flex-wrap:wrap;gap:6px 10px">
        <span><span style="color:#fff">Edu<span style="color:var(--ec-gold,#D79A21)">Core</span></span>&nbsp;&copy; {{ date('Y') }}</span>
        @if($tenant)
