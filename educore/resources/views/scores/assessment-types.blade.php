...
@push('styles')
<style>
.at-page{display:flex;flex-direction:column;gap:16px}.at-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap}.at-head h2{margin:0;font-size:20px!important;color:var(--midnight)}.at-head p{margin:4px 0 0;color:var(--slate-light);font-size:12px}.at-btn{min-height:40px;display:inline-flex;align-items:center;justify-content:center;gap:6px;border:1px solid var(--border);border-radius:8px;padding:0 14px;background:white;color:var(--midnight);font:inherit;font-size:12px;font-weight:800;text-decoration:none;cursor:pointer}.at-btn-primary{background:var(--brand-gold);border-color:var(--brand-gold);color:var(--brand-navy)}.at-btn-danger{background:#FEF2F2;border-color:#FECACA;color:#991B1B}.at-card{background:white;border:1px solid var(--border);border-radius:12px;overflow:hidden}.at-section-head{background:var(--brand-navy);color:white;padding:12px 16px;border-bottom:2px solid var(--brand-gold);display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap}.at-section-head strong{font-size:13px}.at-section-head small{display:block;color:#CBD5E1;font-size:11px;margin-top:2px}.at-body{padding:16px}.at-filter{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:10px;align-items:end;padding:14px 16px}.at-field label{display:block;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.04em;color:var(--slate-light);margin-bottom:5px}.at-field input,.at-field select,.at-field textarea{width:100%;box-sizing:border-box;border:1px solid var(--border);border-radius:8px;background:#F8FAFC;font:inherit;font-size:13px}.at-field input,.at-field select{height:40px;padding:0 10px}.at-field textarea{min-height:72px;padding:9px 10px}.at-table-wrap{overflow-x:auto}.at-table{width:100%;border-collapse:collapse;min-width:760px}.at-table th{background:#F8FAFC;color:var(--slate-light);font-size:10px;text-transform:uppercase;letter-spacing:.04em;padding:10px 12px;text-align:left;border-bottom:1px solid var(--border)}.at-table td{padding:11px 12px;border-bottom:1px solid var(--border);font-size:12px;vertical-align:middle}.at-table tr.selected{background:var(--brand-gold-light)}.at-name{font-weight:800;color:var(--midnight)}.at-sub{font-size:11px;color:var(--slate-light);margin-top:2px;line-height:1.45}.at-badge{display:inline-flex;border-radius:999px;padding:3px 8px;font-size:10px;font-weight:900}.at-active{background:#ECFDF5;color:#047857}.at-draft{background:#F1F5F9;color:#64748B}.at-row-actions{display:flex;gap:6px;flex-wrap:wrap}.at-layout{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(280px,.85fr);gap:16px}.at-total{display:flex;justify-content:space-between;align-items:center;padding-top:12px;margin-top:12px;border-top:1px solid var(--border);font-weight:900}.at-weight{display:inline-flex;border-radius:999px;background:var(--brand-gold-light);color:var(--brand-gold-dark);padding:5px 9px;font-size:11px;font-weight:900}.at-component{border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:10px}.at-component-top{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.at-component-grid{display:grid;grid-template-columns:1.4fr .7fr 1fr 1fr .7fr .7fr auto;gap:8px;align-items:end;margin-top:10px}.at-component-grid .at-field{min-width:0}.at-builder{border:1px dashed #CBD5E1;border-radius:10px;padding:14px;margin-top:14px;background:#FCFDFE}.at-builder-grid{display:grid;grid-template-columns:1.5fr .7fr 1fr 1fr .7fr .7fr;gap:8px}.at-alert{border-radius:9px;padding:10px 13px;font-size:12px}.at-alert-success{background:#ECFDF5;border:1px solid #A7F3D0;color:#065F46}.at-alert-error{background:#FEF2F2;border:1px solid #FECACA;color:#991B1B}.at-alert-warn{background:#FFFBEB;border:1px solid #FDE68A;color:#92400E}.at-meta{display:grid;grid-template-columns:1fr 1fr;gap:10px}.at-checks{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:7px;margin-top:8px}.at-check{display:flex;align-items:center;gap:7px;border:1px solid var(--border);border-radius:8px;padding:8px 10px;background:#F8FAFC;font-size:11px}.at-check input{width:auto}.at-modal-backdrop{position:fixed;inset:0;background:rgba(7,30,69,.45);display:none;align-items:center;justify-content:center;z-index:1000;padding:16px}.at-modal-backdrop.open{display:flex}.at-modal{width:min(620px,100%);max-height:92vh;overflow:auto;background:white;border-radius:14px;box-shadow:0 24px 70px rgba(0,0,0,.24)}.at-modal-head{display:flex;justify-content:space-between;align-items:center;padding:14px 18px;border-bottom:1px solid var(--border)}.at-modal-body{padding:16px 18px}.at-empty{text-align:center;padding:28px 16px;color:var(--slate-light);font-size:12px}@media(max-width:980px){.at-layout{grid-template-columns:1fr}.at-component-grid,.at-builder-grid{grid-template-columns:1fr 1fr}.at-filter{grid-template-columns:1fr 1fr}}@media(max-width:640px){.at-filter,.at-meta,.at-component-grid,.at-builder-grid{grid-template-columns:1fr}.at-head .at-btn{width:100%}.at-row-actions{flex-direction:column}.at-row-actions .at-btn{width:100%}}
</style>
@endpush
...
@php
    ...
    $structureLocked = $selectedTemplate?->assignments->isNotEmpty() ?? false;
...
<div class="at-page">
    <div class="at-head">
        <div>
            ...
            <p>Each school defines its own assessment components, weights and score-entry method, then assigns the completed template to class levels.</p>
        </div>
    ...
    <div class="at-card">
        ...
        <div class="at-table-wrap">
            <table class="at-table">
                <thead><tr><th>Template</th><th>Components</th><th>Total</th><th>Assigned To</th><th>Status</th><th>Action</th></tr></thead>
                <tbody>
                ...
                @empty
                    <tr><td colspan="6"><div class="at-empty">No assessment templates yet. Create a template, then add the components your school uses.</div></td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="at-layout">
        <div class="at-card">
            <div class="at-section-head">
                <div><strong>{{ $selectedTemplate->name }} — Components</strong><small>Schools can define CA, tests, practicals, examinations or any custom-named component.</small></div>
                <span class="at-weight">{{ rtrim(rtrim(number_format($selectedTotal,2),'0'),'.') }} / 100%</span>
            </div>
            <div class="at-body">
                @if($structureLocked)
                    <div class="at-alert at-alert-warn" style="margin-bottom:12px">This template has already been assigned. Its component structure is locked to protect existing score records. Duplicate the template if the school needs a different structure.</div>
                @elseif($selectedTotal < 100)
                    <div class="at-alert at-alert-warn" style="margin-bottom:12px">The template is still being built. Add school-defined components until the total reaches exactly 100%, then activate it.</div>
                @endif

                @forelse($selectedTemplate->components as $component)
                    <div class="at-component">
                        <div class="at-component-top">
                            <div><div class="at-name">{{ $component->name }}</div><div class="at-sub">{{ str($component->component_type)->replace('_',' ')->title() }} · {{ str($component->entry_mode)->replace('_',' ')->title() }}</div></div>
                        ...
                        <form method="POST" action="{{ route('assessment-templates.components.update',[$selectedTemplate,$component]) }}" class="at-component-grid">
                            @csrf @method('PUT')
                            <div class="at-field"><label>Name</label><input name="name" value="{{ $component->name }}" required></div>
                            ...
                            <div class="at-field"><label>Entry Mode</label><select name="entry_mode">@foreach(['manual'=>'Manual','cbt_objective'=>'CBT Objective','cbt_aggregate'=>'CBT Aggregate','theory_manual'=>'Theory Manual'] as $value=>$label)<option value="{{ $value }}" @selected($component->entry_mode===$value)>{{ $label }}</option>@endforeach</select></div>
                            <div class="at-field"><label>Objective Max</label><input type="number" step="0.01" min="0.01" name="objective_max" value="{{ $component->objective_max }}" placeholder="Optional"></div>
                            <div class="at-field"><label>Theory Max</label><input type="number" step="0.01" min="0.01" name="theory_max" value="{{ $component->theory_max }}" placeholder="Optional"></div>
                ...
                <div class="at-builder">
                    <div class="at-name">Add School Component</div>
                    <div class="at-sub">Component names are school-defined. Examples: 1st CA, Weekly Test, Practical, Project, Mock, Final Examination.</div>
                    <form method="POST" action="{{ route('assessment-templates.components.store',$selectedTemplate) }}" style="margin-top:10px">@csrf
                        <div class="at-builder-grid">
                            <div class="at-field"><label>Component Name</label><input name="name" required placeholder="e.g. 1st CA"></div>
                            <div class="at-field"><label>Weight %</label><input type="number" step="0.01" min="0.01" max="100" name="weight_percentage" required placeholder="20"></div>
                            ...
                            <div class="at-field"><label>Entry Mode</label><select name="entry_mode"><option value="manual">Manual</option><option value="cbt_objective">CBT Objective</option><option value="cbt_aggregate">CBT Aggregate</option><option value="theory_manual">Theory Manual</option></select></div>
                            <div class="at-field"><label>Objective Max</label><input type="number" step="0.01" min="0.01" name="objective_max" placeholder="Optional"></div>
                            <div class="at-field"><label>Theory Max</label><input type="number" step="0.01" min="0.01" name="theory_max" placeholder="Optional"></div>
                        </div>
        ...
        <div style="display:flex;flex-direction:column;gap:16px">
            <div class="at-card">
                <div class="at-section-head"><div><strong>Template Settings</strong><small>Activate only after components total 100%.</small></div></div>
                <div class="at-body">
            ...
            <div class="at-card">
                <div class="at-section-head"><div><strong>Assignment</strong><small>Apply the template to class levels for a session.</small></div></div>
                <div class="at-body">
            ...
            <div class="at-card">
                <div class="at-section-head"><div><strong>Current Usage</strong><small>Class levels already linked to this template.</small></div></div>
                <div class="at-body">
    ...
    <div class="at-modal">
        ...
        <form method="POST" action="{{ route('assessment-templates.store') }}">@csrf
            <div class="at-modal-body">
                <div class="at-field"><label>Template Name</label><input name="name" required placeholder="e.g. Senior Secondary Standard"></div>
                ...
                <div class="at-alert at-alert-warn" style="margin-top:12px">The template starts as Draft. After creating it, the school can add any number of assessment components until their weights total 100%.</div>
