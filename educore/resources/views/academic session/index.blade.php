...
@if($errors->has('error'))
    <div class="alert-e">&#9888; {{ $errors->first('error') }}</div>
@endif
...
<div class="ac-banner">
    ...
    <div>
        ...
        @else
            ...
            <div class="ac-status-sub">
                @if($currentSession)
                    Create a term for {{ $currentSession->name }}
                @else
                    Activate a session first
                @endif
            </div>
        @endif
    </div>
        ...
        <tr>
            ...
            <td>
                <div class="ac-actions">
                    ...
                    @if(!$session->is_current)
                    <form method="POST" action="{{ route('academic-cycle.sessions.destroy', $session) }}" style="display:inline" onsubmit="return confirm('Delete this academic session? This action cannot be undone.')">@csrf @method('DELETE')<button type="submit" class="btn-sm-act btn-del">&#128465;</button></form>
                    @endif
                </div>
            </td>
        </tr>
...
{{-- TERMS --}}
<div class="ac-term-column">
<div class="ac-card">
    <div class="ac-card-head">
        <span class="ac-card-title">&#128203; New Term</span>
        @if(!$currentSession)
            <span style="font-size:11px;color:#D97706;font-weight:600">&#9888; Activate a session first</span>
        @endif
    </div>
    <div class="ac-card-body">
...
<div class="ac-card ac-terms-card">
    <div class="ac-card-head">
        ...
        <tr>
            ...
            <td>
                <div class="ac-actions">
                    ...
                    @else
                    <form method="POST" action="{{ route('academic-cycle.terms.close', $term) }}" style="display:inline" onsubmit="return confirm('Close this term?')">@csrf<button type="submit" class="btn-sm-act btn-close" title="Close">&#10005;</button></form>
                    @endif
                    <button type="button" class="btn-sm-act btn-edit" onclick="toggleEdit('term',{{ $term->id }})">&#9998;</button>
                    @if(!$term->is_current)
                    <form method="POST" action="{{ route('academic-cycle.terms.destroy', $term) }}" style="display:inline" onsubmit="return confirm('Delete this term?')">@csrf @method('DELETE')<button type="submit" class="btn-sm-act btn-del">&#128465;</button></form>
                    @endif
                </div>
            </td>
        </tr>
...
function toggleEdit(type, id) {
    ...
    document.querySelectorAll('.edit-row.open').forEach(function (openRow) {
