...
<style>
    ...
    .profile-grid { display:grid;grid-template-columns:280px minmax(0,1fr);gap:16px;min-width:0; }
    ...
    .info-row { display:flex;justify-content:space-between;gap:14px;padding:9px 0;border-bottom:1px solid var(--border);font-size:13px; }
    .info-row:last-child { border-bottom:none; }
    .info-key { color:var(--slate);font-weight:500; }
    .info-val { color:var(--midnight);font-weight:600;text-align:right;overflow-wrap:anywhere; }
    ...
    .form-control { width:100%;min-width:0;max-width:100%;padding:9px 12px;font-size:13px;font-family:inherit;border:1px solid var(--border);border-radius:8px;background:#F8FAFC;outline:none;transition:border-color 200ms; }
    .form-control:focus { border-color:var(--indigo);box-shadow:0 0 0 3px rgba(37,99,235,0.1);background:white; }
    .btn { display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:9px 16px;font-size:13px;font-weight:600;font-family:inherit;border-radius:8px;border:none;cursor:pointer;text-decoration:none;transition:background 150ms;min-height:40px; }
    ...
    @media(max-width:520px) { .card-body{padding:16px}.info-row{align-items:flex-start}.form-actions{display:grid;grid-template-columns:1fr}.form-actions .btn{width:100%} }
</style>
@endpush
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="info-row"><span class="info-key">Status</span>
                    <span class="info-val" style="color:{{ $staff->is_active ? 'var(--emerald)' : 'var(--slate)' }}">{{ $staff->employmentStatusLabel() }}</span>
                </div>
