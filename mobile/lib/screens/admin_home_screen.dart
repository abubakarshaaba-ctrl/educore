  ...
  Widget build(BuildContext context) => Column(children: [
        ...
        Expanded(
          ...
          builder: (data) {
            ...
            if (items.isEmpty) {
              return const _Empty(
                  icon: Icons.groups_outlined, text: 'No records available.');
            }
            return RefreshIndicator(
              onRefresh: () async => setState(() => _future = _load()),
              child: ListView.builder(
                ...
                itemBuilder: (_, index) {
                  ...
                  return _AdminTile(
                    icon:
                        _students ? Icons.person_outline : Icons.badge_outlined,
...
class _AcademicsScreenState extends State<_AcademicsScreen> {
  late Future<Map<String, dynamic>> _future =
      ApiClient.instance.get('/admin/academics');
  @override
  Widget build(BuildContext context) => _AdminFuture(
        future: _future,
        onRetry: () => setState(
...
class _FinanceScreenState extends State<_FinanceScreen> {
  late Future<Map<String, dynamic>> _future =
      ApiClient.instance.get('/admin/finance');
  @override
  Widget build(BuildContext context) => _AdminFuture(
        future: _future,
        onRetry: () =>
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      const CircleAvatar(
          ...
          child:
              Icon(Icons.admin_panel_settings_rounded, color: kGold, size: 32)),
      const SizedBox(height: 12),
      Text(user['name']?.toString() ?? 'School Administrator',
          textAlign: TextAlign.center,
          style: const TextStyle(
              color: kInk, fontSize: 19, fontWeight: FontWeight.w800)),
      Text('${user['role'] ?? 'Administrator'} · $school',
      ...
      const _AdminTile(
          ...
          subtitle:
              'Only modules granted to your administrator role are visible.'),
      const _AdminTile(
          icon: Icons.verified_user_outlined,
          title: 'Secure school workspace',
          subtitle: 'School data remains isolated to your institution.'),
      const SizedBox(height: 18),
      OutlinedButton.icon(
        onPressed: () async {
...
class _AdminHero extends StatelessWidget {
  const _AdminHero(
      {required this.name, required this.role, required this.period});
  final String name;
  final String role;
  final String period;
  @override
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Flexible(
                child: Text('$role · Authorized modules only',
                    style: const TextStyle(
                        color: Colors.white,
                        fontSize: 12,
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListTile(
      ...
      trailing: Text(value,
          style: const TextStyle(
              color: kNavy, fontSize: 17, fontWeight: FontWeight.w800)));
}
...
class _AdminFuture extends StatelessWidget {
  const _AdminFuture(
      {required this.future, required this.onRetry, required this.builder});
