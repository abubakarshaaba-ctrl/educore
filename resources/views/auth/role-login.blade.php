...
@section('page-title', $meta['eyebrow'] . ' - EduCore')
...
<div class="auth-shell auth-shell--refined" style="--tenant-primary: var(--ec-navy); --tenant-accent: var(--ec-gold);">
    ...
    <main class="auth-panel">
        <section class="auth-card" aria-labelledby="role-login-heading">
            <div class="auth-portal-context">
                <span class="auth-portal-context__icon" aria-hidden="true">
                    ...
                    @elseif($surface === 'staff')
                        <svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="4" stroke="currentColor" stroke-width="1.7"/><path d="M4 21c0-4.4 3.6-8 8-8s8 3.6 8 8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>
                    @elseif($surface === 'student')
                        <svg viewBox="0 0 24 24" fill="none"><path d="m3 9 9-5 9 5-9 5-9-5Z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><path d="M6 12v5l6 3 6-3v-5" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/></svg>
                    @else
                        <svg viewBox="0 0 24 24" fill="none"><circle cx="9" cy="8" r="3" stroke="currentColor" stroke-width="1.7"/><circle cx="17" cy="10" r="2.5" stroke="currentColor" stroke-width="1.7"/><path d="M3 20c0-4 2.7-7 6-7s6 3 6 7M14 15c3.4 0 6 2.2 6 5" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/></svg>
                    @endif
                </span>
                <span>
                    <span class="auth-portal-context__label">{{ $meta['eyebrow'] }}</span>
                    <span class="auth-portal-context__meta">Secure EduCore access</span>
                </span>
            </div>

            <header class="auth-card__header">
                <h2 class="auth-title" id="role-login-heading">Welcome back</h2>
                <p class="auth-subtitle">{{ $meta['lead'] }}</p>
            </header>
            ...
            <form method="POST" action="{{ route($surface . '.login') }}" novalidate>
                @csrf
                <div class="ec-form-group">
                    <label class="ec-label" for="login_id">{{ $meta['id_label'] }}</label>
                    <input id="login_id" class="ec-input{{ $errors->has('login_id') ? ' ec-input--error' : '' }}"
                        type="text" name="login_id" value="{{ old('login_id') }}"
                        autocomplete="username" required placeholder="Enter {{ strtolower($meta['id_label']) }}"
            ...
            <nav class="auth-portal-switcher" aria-label="Other EduCore portals">
                <p class="auth-portal-switcher__title">Continue through another portal</p>
                <div class="auth-portal-switcher__links">
                    @if($surface !== 'admin')
                        <a class="auth-portal-switcher__link" href="{{ route('admin.login') }}"><span>Administrator</span><svg viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                    @endif
                    @if($surface !== 'staff')
                        <a class="auth-portal-switcher__link" href="{{ route('staff.login') }}"><span>Staff</span><svg viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                    @endif
                    @if($surface !== 'student')
                        <a class="auth-portal-switcher__link" href="{{ route('student.login') }}"><span>Student</span><svg viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                    @endif
                    @if($surface !== 'parent')
                        <a class="auth-portal-switcher__link" href="{{ route('parent.login') }}"><span>Parent</span><svg viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
                    @endif
                </div>
            </nav>

            @if(in_array($surface, ['admin', 'staff']))
