...
.stat-card.early{border-top-color:#0284C7}.stat-card.present{border-top-color:var(--emerald,#059669)}.stat-card.late{border-top-color:var(--amber,#D97706)}.stat-card.absent{border-top-color:var(--crimson,#DC2626)}
.stat-val{font-size:28px;font-weight:800}.stat-lbl{font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin-top:3px}
...
.b-early{background:#E0F2FE;color:#0284C7}
...
.nav-tab.active{background:#2563EB;border-color:#2563EB;color:white}
...
<div style="display:flex;gap:4px;margin-bottom:20px;flex-wrap:wrap">
    ...
    <a href="{{ route('staff-attendance.offline-queue') }}"
       class="nav-tab" style="color:var(--amber,#D97706)">
        📡 Offline Queue
    </a>
    @endif
    @endif
...
<div class="stats-row">
    ...
    <div class="stat-card present"><div class="stat-val" style="color:var(--emerald,#059669)">{{ $counts['present'] }}</div><div class="stat-lbl">🟢 Present</div></div>
    <div class="stat-card late"><div class="stat-val" style="color:var(--amber,#D97706)">{{ $counts['late'] }}</div><div class="stat-lbl">🟡 Late</div></div>
    <div class="stat-card absent"><div class="stat-val" style="color:var(--crimson,#DC2626)">{{ $counts['absent'] }}</div><div class="stat-lbl">🔴 Absent</div></div>
</div>
...
<div class="card" style="margin-bottom:16px">
    <div style="padding:14px 18px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            ...
            <span style="font-size:16px;font-weight:800;color:{{ $punctuality >= 80 ? 'var(--emerald,#059669)' : ($punctuality >= 60 ? 'var(--amber,#D97706)' : 'var(--crimson,#DC2626)') }}">{{ $punctuality }}%</span>
        </div>
        <div class="progress-bar">
            <div class="progress-fill" style="width:{{ $punctuality }}%;background:{{ $punctuality >= 80 ? 'var(--emerald,#059669)' : ($punctuality >= 60 ? 'var(--amber,#D97706)' : 'var(--crimson,#DC2626)') }}"></div>
        </div>
        <div style="font-size:11px;color:var(--muted);margin-top:4px">On-time arrivals (Early + Present) ÷ total working days</div>
    </div>
</div>
...
<div class="card">
    ...
    <div class="tbl"><table>
        ...
        <tr>
            ...
            <td style="font-family:monospace;color:var(--muted)">{{ $rec->clock_out_time ? \Carbon\Carbon::parse($rec->clock_out_time)->format('H:i') : '—' }}</td>
            ...
            <td style="font-size:11px;color:var(--muted)">
                ...
                @if($rec->geo_verified)<span style="color:var(--emerald,#059669)">📍</span>@endif
            </td>
        </tr>
        @empty
        <tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted)">No attendance records for this month.</td></tr>
        @endforelse
        </tbody>
    </table></div>
</div>
  ...
  <div id="proxyStep2" style="display:none;padding:16px 20px 20px">
    ...
    <div id="pqrCameraDiv">
      ...
      <div id="proxyQrHttpWarn" style="display:none;font-size:11px;color:#D97706;background:#FFFBEB;border:1px solid #FDE68A;border-radius:7px;padding:8px 10px;margin-bottom:8px;line-height:1.5">
        ⚠️ Camera unavailable. Use <strong>Manual</strong> mode.
      </div>
    </div>
...
<div id="scanModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:1000;align-items:center;justify-content:center">
    ...
    <div id="cameraModeDiv">
        ...
        <div id="httpWarning" style="display:none;font-size:11px;color:#D97706;background:#FFFBEB;border:1px solid #FDE68A;border-radius:7px;padding:8px 10px;margin-bottom:8px;line-height:1.5">
            ⚠️ <span id="httpWarningMsg">Camera blocked. Use <strong>Upload Photo</strong> or <strong>Manual</strong> mode instead.</span>
            <div style="margin-top:6px">
                <button onclick="retryCameraScanner()" style="padding:4px 10px;font-size:11px;font-weight:700;background:#2563EB;color:white;border:none;border-radius:6px;cursor:pointer;font-family:inherit">↺ Retry Camera</button>
            </div>
        </div>
    </div>
...
async function startCameraScanner() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        showCameraError('Camera API not available. Use <strong>Upload Photo</strong> or <strong>Manual</strong> mode instead.');
        return;
    }
    try {
        stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment', width:{ideal:1280}, height:{ideal:720} } });
        startCameraStream(video, stream);
    } catch(e) {
        ...
        let msg = 'Camera unavailable. Use <strong>Upload Photo</strong> or <strong>Manual</strong> mode instead.';
        if (e.name === 'NotAllowedError') {
            msg = 'Camera permission denied. Tap the 🔒 lock icon in your browser address bar → Site settings → Camera → Allow, then tap ↺ Retry Camera.';
        } else if (e.name === 'NotFoundError') {
            msg = 'No camera found. Use <strong>Upload Photo</strong> or <strong>Manual</strong> mode instead.';
        } else if (e.name === 'NotReadableError') {
            msg = 'Camera is in use by another app. Close it then tap ↺ Retry Camera.';
        }
        showCameraError(msg);
...
function startCameraStream(video, s) {
    video.srcObject = s;
    video.play();
...
function showCameraError(msg) {
    document.getElementById('httpWarningMsg').innerHTML = msg;
...
async function startFriendCamera() {
    ...
    try {
        ...
        document.getElementById('friendPhotoUnavailable').style.display = 'none';
        document.getElementById('friendStartCamBtn').style.display = 'none';
        let camErrMsg = 'Camera unavailable. Upload a photo instead.';
        if (e.name === 'NotAllowedError') camErrMsg = 'Camera permission denied. Tap the lock icon in the address bar to allow camera access.';
        else if (e.name === 'NotFoundError') camErrMsg = 'No camera found on this device. Upload a photo instead.';
        else if (e.name === 'NotReadableError') camErrMsg = 'Camera is in use by another app. Close it and try again.';
        document.getElementById('friendCameraError').textContent = camErrMsg;
...
async function startProxyQrCamera() {
    ...
    try {
        ...
        let warnMsg = 'Camera unavailable. Use <strong>Manual</strong> mode.';
        if (e.name === 'NotAllowedError') warnMsg = 'Camera permission denied. Tap the lock icon in the address bar to allow, then retry.';
        else if (e.name === 'NotReadableError') warnMsg = 'Camera is in use by another app. Close it and try again.';
        document.getElementById('proxyQrHttpWarn').innerHTML = '⚠️ ' + warnMsg;
        document.getElementById('proxyQrHttpWarn').style.display = 'block';
        document.getElementById('proxyQrStatus').textContent = '';
    }
}
